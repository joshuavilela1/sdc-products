...
module.exports = {
  ...
  getProductsDB: (page, count, cb) => {
    ...
    pool.query(`SELECT * FROM products WHERE id > ${(page - 1) * count} AND id < ${(page * count) + 1}`)
  ...
  getProductInfoDB: (product_id, cb) => {
    pool.query(`SELECT *, (SELECT jsonb_agg(features) FROM (SELECT feature, value FROM features WHERE
  ...
  getStylesDB: (product_id, cb) => {
    pool.query(`SELECT id AS product_id,
